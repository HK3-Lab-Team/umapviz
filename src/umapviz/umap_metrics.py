...
@nb.njit()
def rogers_tanimoto_weighted(x, y, weights: np.ndarray):
    num_not_equal = 0.0
    for i in range(x.shape[0]):
        x_true = x[i] != 0
        y_true = y[i] != 0
        num_not_equal += weights[i] * (x_true != y_true)

    return (2.0 * num_not_equal) / (np.sum(weights) * (x.shape[0] + num_not_equal))
...
def tanimoto_gower(
    ...
    Arguments:
        a {np.ndarray} -- 1d array of features
        b {np.ndarray} -- 1d array of features
        min_vals {np.ndarray} -- 1d array min values in sample
    ...
    if boof_in:
        a_m, b_m = a[boolean_features], b[boolean_features]
        weights_bool = feature_weights[boolean_features]
        bool_dist = rogers_tanimoto_weighted(a_m, b_m, weights_bool)
    ...
    if catf_in and numf_in:
        ...
        if catf_in:
            gower_dist = gw_cat_dist / weights_cat.sum()
        elif numf_in:
