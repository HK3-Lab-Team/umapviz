...
class FeatureCategoriesList:
    ...
    def __iter__(self) -> Iterator[FeatureCategory]:
        ...
        maintaining the same order so every step or list deriving from these is created
        with the same order.

        Returns
        -------
        attr, value: Tuple[str, FeatureCategory]
            Iterator that yields an ordered list of tuples (key, value), where the 'key'
            is the name of the attribute of this class, and 'value' is the corresponding
            FeatureClass instance
        """
...
class UmapExperiment:
    """
    This class gathers all the settings required to build UMAP experiment with optional
    clustering and plot it.
    """
    ...
    def __init__(
        ...
        """
        This constructor gathers all the settings and methods required to build UMAP
        experiment with optional clustering and plot it.

        Parameters
        ----------
        df_info: DataFrameWithInfo
            Full DataFrameWithInfo which contains all the data and info to be used for
            UMAP and clustering
        n_neighbors: int
            Number of neighbors to consider in UMAP algorithm. It depends on the number
            of samples available
        min_distance: float
            Minimum distance to consider in UMAP algorithm. It depends on which scale
            you want to see patterns/structures
        random_seed: int
            Numeric value to fix the initial random state in order to get repeatable
            results from UMAP algorithm
        exclude_feat_list: Tuple
            Tuple of the feature names (df_info columns) that you want to exclude.
            Default set to ()
        numer_feat_list: Tuple
            Tuple of the feature names (df_info columns) containing numerical values
            only. If None, 'df_info.column_list_by_type.numerical_cols' attribute will
            be used. Default set to None
        categ_feat_list: Tuple
            Tuple of the feature names (df_info columns) containing categorical values
            only. If None, 'df_info.column_list_by_type.categorical_cols' attribute will
            be used. Default set to None
        bool_feat_list: Tuple
            Tuple of the feature names (df_info columns) containing boolean values only.
            If None, 'df_info.column_list_by_type.bool_cols' attribute will be used.
            Default set to None
        not_nan_percentage_threshold: float
            Every feature, which will contain less than this percentage of not-NaN,
            will not be considered in the returned data used to compute embeddings by
            UMAP
        train_test_split_ratio: float
            This specifies the ratio between training and test set. It corresponds to
            the size of test set divided by the size of the whole set
        feature_to_color: str
        ...
        multi_marker_feats: Tuple
            These column_names/features will be used to define different series of
            points (described and listed in legend). Their unique values combination
            will define different partitions that will be distinguishable by markers
        enc_value_to_str_map: Dict[Dict]
            This is a map to connect the encoded values to the original ones. It will be
            a Dict of Dicts because the first level keys identify the features with
            encoded values, and the second level dict is the actual map
        file_title_prefix: str
            This string will be used as prefix in the name of the exported bokeh plot
            file and in the plot title
        metric: function
            This is a function which calculates the distances between two vectors
            according to some metric_kwds that can be defined and passed. This function
            should be written according to UMAP documentations and as a numba.njit
            function
        metric_kwds:
            Keywords used by the metric that will be used for clustering or UMAP
            algorithm computations. If not set, the metric_kwds will be set for
            'tanimoto_gower' custom metric.
        feature_weights: Tuple[Float]
            This tuple will be used in metric to give different weights for every
            feature. This can be None and it can be set later or, if not provided,
            weights per category (e.g. numer_feat_weight) will be used.
        ...
        group_values_to_be_shown: Tuple[str, Tuple]
            This is required if you want to select only a part of DataFrame as a Series
            of grey points (as background), and the other part will be split in
            partitions according to the other arguments. If you want the DataFrame to
            be considered all together with no grey/background points, set this to None
            (default value). This is a tuple where the first element is the name of the
            column which you want to set a condition on. The second element is a Tuple
            containing all the values from that column that have to be included in the
            plot. The rows that have other values for that column will be plotted as
            grey points. E.g. ('BREED', (('LABRADOR RETRIEVER',), ('MONGREL',))) ->
            Two plots will be created where these two breeds will separately be plotted
            as colored points, while the other breeds will be plotted as grey points
        color_tuple
        test_color_tuple
        tools
        tooltip_feats:
        tooltips: Tuple[Tuple[str, str]]
            If not specified, the tooltips will be defined by using the name of the
            features in 'tooltip_feats' argument. If these names are not clear, it can
            be useful to manually specify the tooltip tuples by using this argument
        marker_fill_alpha: float
            Float number in range [0, 1] that describes the transparency of the marker
            fill_color (1 is opaque, 0 transparent)
        marker_size: int
        ...
        # will be set based on these. If not present weights per category (e.g.
        ...
        # 'self._train_umap_data' and 'self._train_notna_full_features_df' will be
        # used even if 'train_test_split_ratio' = 0
        ...
        for feat_class in self._feat_list:
            for col in feat_class.list_f:
                try:
                    ...
                    logger.error(
                        f"Column {col} from {feat_class.name_f} columns is not present"
                        " in df_info"
                    )
    ...
    def _remove_nan(self) -> pd.DataFrame:
        ...
        The features with too many NaN will not be considered in UMAP plot and they will
        be removed from self.feat_list attributes (list of features by category)

        Then remove the rows with any NaN left, because UMAP will not handle any NaN
        value (distance would not be defined)

        Returns
        -------
        umap_data: pandas.DataFrame
            This is a slice of 'self.df_info.df' containing only the features to be used
            for UMAP fitting (with no NaN and only numerical/categorical/bool values)
        """
        ...
        for feat_list in self._feat_list:
            feat_list_notnan = []
            # For each feature analyze the number of NaN and keep the ones that have a
            # number of not-NaN higher than 'self.not_nan_percentage_threshold'
            for f in feat_list.list_f:
                try:
                    ...
                    logger.warning(
                        f"The {feat_list.name_f} feature {f} is not present in "
        ...
        # Select only the rows based on "feat_list_notnan", dropping the ones containing
        # NaN
    ...
    def _train_test_split(
        ...
        It also returns the train and test set with all the features, even the ones that
        are not used in UMAP fit computation.
        ...
        -------
            ...
            train_notna_full_features_df: DataFrameWithInfo
                This is a DataFrameWithInfo with the same samples as train_umap_data,
                but with more features because the ones with many NaN have not been
                dropped
    ...
    def _validate_features(self):
        """
        The function will validate the features in order to verify that the features
        marked in the plot (i.e. "feature_to_color" or
    ...
    def fit_transform(
        ...
        Then these will be passed to UMAP algorithm for fitting on train set. The train
        and test set embeddings will be computed and returned
        ...
        repeat_fitting: bool
            This option is to choose to repeat the UMAP fitting using the selected
            umap_components
        min_distance: float
        n_neighbors: int
        ...
        test_embedding: np.ndarray
            UMAP embeddings of the test samples (computed by using the UMAP fit on
            train set)
        """
        if (
            repeat_fitting
